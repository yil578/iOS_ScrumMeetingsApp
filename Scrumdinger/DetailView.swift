    ...
    var body: some View {
        List {
            Section(header: Text("Meeting Info")) {
                NavigationLink(destination: MeetingView(scrum: $scrum)) { //1st Navigation in DetailView (to MeetingView)
            ...
            Section(header: Text("History")) {
                ...
                ForEach(scrum.history) { history in
                    HStack {
                        ...
                        Text(history.date, style: .date) // Use HStack to display a calendar icon and the date for each history in the array
                    }
                }
            }
        }
